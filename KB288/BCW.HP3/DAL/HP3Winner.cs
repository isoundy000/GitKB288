...
namespace BCW.HP3.DAL
{
    ...
    #region  成员方法
        ...
        public bool Exists(int ID)
        {
            ...
            strSql.Append("select count(1) from tb_HP3Winner");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into tb_HP3Winner(");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update tb_HP3Winner set ");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update tb_HP3Winner ");
        ...
        public bool Delete(int ID)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete from tb_HP3Winner ");
            strSql.Append(" where ID=@ID");
            SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)
			};
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select ID,WinUserID,WinDate,WinMoney,WinBool,WinZhu ");
            strSql.Append(" FROM tb_HP3Winner ");
            if (strWhere.Trim() != "")
            {
        ...
        public BCW.HP3.Model.HP3Winner GetModel(int ID)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  top 1 ID,WinUserID,WinDate,WinMoney,WinBool,WinZhu from tb_HP3Winner ");
            strSql.Append(" where ID=@ID ");
            SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)			};
            parameters[0].Value = ID;
        ...
        /// 得到一个对象实体
        /// </summary>
        public BCW.HP3.Model.HP3Winner DataRowToModel(DataRow row)
        {
            BCW.HP3.Model.HP3Winner model = new BCW.HP3.Model.HP3Winner();
            if (row != null)
            {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  WinUserID,sum(WinMoney) AS'sm' into #bang from tb_HP3Winner  group by WinUserID order by sum(WinMoney)");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  WinUserID,sum(WinMoney) AS'sm' into #bang3 from tb_HP3Winner   where WinDate>='" + s1 + "' and WinDate<='" + s2 + "'  group by WinUserID order by sum(WinMoney)");
            strSql.Append("SELECT * FROM ( ");
            strSql.Append(" SELECT ROW_NUMBER() OVER (");
            strSql.Append("order by T.sm desc");
        ...
        {
            IList<BCW.HP3.Model.HP3Winner> listHP3 = new List<BCW.HP3.Model.HP3Winner>();
            string sTable = "tb_HP3Winner";
            string sPkey = "ID";
            ...
            using (SqlDataReader reader = SqlHelper.RunProcedureMe(sTable, sPkey, sField, p_pageIndex, p_pageSize, sCondition, sOrder, iSCounts, out p_recordCount))
            {
                ...
                while (reader.Read())
                {
                    BCW.HP3.Model.HP3Winner objHP3 = new BCW.HP3.Model.HP3Winner();
       ...
       //查询我的中奖未兑换订单
        public DataSet GetMyWinList(string strWhere)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select * ");
            strSql.Append(" from tb_HP3Winner inner join tb_HP3Buy on tb_HP3Winner.ID=tb_HP3Buy.ID  ");
            if (strWhere.Trim() != "")
            {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select sum(WinMoney) from  tb_HP3Winner");
