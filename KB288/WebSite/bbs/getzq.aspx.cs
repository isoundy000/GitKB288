...
/// <summary>
/// 20160802 姚志光 更新到服务器
/// 20160809 姚志光 修复已完场的球赛不再更新
/// </summary>
public partial class bbs_getzq : System.Web.UI.Page
{
    private string ex = "";
    string getZQOpen = (ub.GetSub("getZQOpen", "/Controls/footballs.xml"));
    string getLQOpen = (ub.GetSub("getLQOpen", "/Controls/footballs.xml"));
    string BasketBallHidden = (ub.GetSub("BasketBallHidden", "/Controls/footballs.xml"));
    protected void Page_Load(object sender, EventArgs e)
    {
        //  Response.Write("<meta http-equiv=\"refresh\" content=\"80\" />");
        Response.Write(DateTime.Now.AddDays(-1).ToString("yyyy-MM-dd"));
        #region  篮球捉取 开关与开始
        if (getLQOpen == "1")
        {
            try
            {
                ...
                try
                {
                    ...
                    url[7] = "http://bf.win007.com/NBA/today.xml";//今日赛程
                    url[8] = "http://bf.win007.com/NBA/today2.xml";//今日赛程
                    url[9] = "http://bf.win007.com/nba_date.aspx?date=" + DateTime.Now.AddDays(-1).ToString("yyyy-MM-dd") + "&h=0&m=3";//一周赛程
                ...
                #endregion
                Response.Write("=========篮球捉取成功！当前(getLQOpen==1)=====ok1ok1ok1ok1ok1====" + "</br>");
            }
            catch { Response.Write("=========篮球捉取已异常动停止！当前(getLQOpen==1)=====error1error1error1error1error1====" + "</br>"); }

        }
        else
        {
            Response.Write("=========篮球捉取已手动停止！当前(getLQOpen==0)=====close1close1close1close1close====" + "</br>");
        }
        #endregion
        ...
        if (getZQOpen == "1")
        {
            ...
            catch (WebException ww)
            {
                Response.Write("=========足球捉取已异常(WebException)动停止！当前(getZQOpen==1)=====error1error1error1error1error1====" + "</br>");
                Response.Write("=========足球捉取异常消息：" + ww.ToString() + "</br>");
            }
            catch (Exception ee)
            {
                Response.Write("=========足球捉取已异常动(Exception)停止！当前(getZQOpen==1)=====error1error1error1error1error1====" + "</br>");
                Response.Write("=========足球捉取异常消息：" + ee.ToString() + "</br>");
            }
        }
        else
        {
        ...
        foreach (XmlNode node in nodelist)
        {
            foreach (XmlNode nodes in node)
            {
                if (nodes.Name == "h" && nodes.InnerText.Length > 10)
                {
                    ...
                    if (model.matchtime > DateTime.Now.AddHours(-6) && model.matchstate != "-1" && model.matchstate != "0" && model.matchstate != "-5")
                    {
                        ...
                        Response.Write("当前直播文字地址:" + texturl + "<br/>");
                    ...
                    string BasketBallHidden = (ub.GetSub("BasketBallHidden", "/Controls/footballs.xml"));
                    if (BasketBallHidden == "1")
                    {
                        model.isHidden = 1;//默认显示  
                    }
                    else
                    {
                        model.isHidden = 0;
                    ...
                    if (BasketBallHidden == "1")
                    {
                        string strWhere = "ID>363514 and p_type=" + 2 + " and p_active=0 and p_del=0 and (p_TPRtime >= '" + System.DateTime.Now + "' OR (p_ison=1 and p_isondel=0))  and p_basketve=0";
                        // 开始读取竞猜
                        ...
                        if (have != null && have.Tables[0].Rows.Count > 0)
                        {
                            ...
                            {
                                if (DT.GetTime(model.matchtime.ToString(), "MM月dd日").ToString() == DT.GetTime(have.Tables[0].Rows[i]["p_TPRtime"].ToString(), "MM月dd日").ToString())
                                {
                                    if (model.hometeam.Contains(have.Tables[0].Rows[i]["p_once"].ToString().Trim()) && model.guestteam.Contains(have.Tables[0].Rows[i]["p_two"].ToString().Trim()))
                                    {
                                        if (model.classType.Contains(have.Tables[0].Rows[i]["p_title"].ToString()))
                                        {
                                            ...
                                            new BCW.BLL.tb_BasketBallList().UpdateHidden(model.ID, model.isHidden);
                                            new BCW.BLL.tb_BasketBallList().UpdateConnectId(model.ID, model.connectId);
                                        }
                                    }
                                }
                            }
                    ...
                    //    if((model.matchstate.ToString().Trim())!="-1")
                    {
                        ...
                        try
                        {
                            ...
                            if (model.ID > 0 || model.name_en > 0)
                            {
                                ...
                                //{
                                //    if (model.matchtime > DateTime.Now.AddHours(3))
                                //    {
                                //        Response.Write(model.ID + "*(" + model.name_en + ")#" + model.matchtime + "#" + model.hometeam + "-" + model.guestteam + "-" + "未开赛,无更新!状态:" + model.matchstate + "<br/>");
                                //    }
                                //    else
                                //    {
                                if (model.matchtime > DateTime.Now.AddDays(-1))//是否已完结
                                {
                                    if (ball.matchstate.Trim() == "-1")
                                    {
                                        Response.Write(model.ID + "*(" + model.name_en + ")#" + model.matchtime + "#" + model.hometeam + "-" + model.guestteam + "-" + "<font color=\"blue\">" + "已完场,无更新!状态:" + model.matchstate + "</font>" + "<br/>");
                                    }
                                    else
                                    {
                                        ...
                                        {
                                            //若正在进行的球赛比分变化
                                            if (model.matchstate != "-1" && model.homescore != ball.homescore || model.guestscore != ball.guestscore)
                                            {
    ...
    public void getMatch()
    {
        ...
        for (int i = 1; i < boTemp.Length; i++)
        {
            ...
            {
                Response.Write("[" + ft.ft_time + "]" + ft.ft_teamStyle + ft.ft_team1 + "--" + ft.ft_team2 + "赛事处理异常！" + "<br/>");
                Response.Write("异常消息：" + eee.ToString());
            }
        }
    }
